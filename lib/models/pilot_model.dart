...
class Pilot {
  ...
  void setHealth(EnumDirection direction) {
    if (direction == EnumDirection.increment) {
      _health++; 
      if (_health > 6) { _health = 6; }
    }
    else {
      _health--;
      if (_health == 0) { throw PilotException(EnumGameOver.health, constGameOverHealth); }
    }
  ...
  void setProximity(EnumDirection direction) {
    if (direction == EnumDirection.increment) {
      ...
      _proximity--;
      if (_proximity == 0) { throw PilotException(EnumGameOver.proximity, constGameOverProximity); }
    }
  ...
  bool hasAffliction(EnumAffliction affliction) {
    ...
    else if ((affliction == EnumAffliction.gunshotwound) && (_hasGunshotWound)) {
      result == true; 
    }
